...
function App() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const breeds = Object.entries(breedListResponse.message).reduce<string[]>((acc, [breed, subbreeds]) => subbreeds.length ? [...acc, ...subbreeds.map((subbreed: string) => `${breed} ${subbreed}`)] : [...acc, breed], [])
        setBreeds(breeds)
      } catch(error) {
        console.error(error)
      }
  ...
  return (
    <main style={{backgroundImage: `url(${backgroundImage})`}}>
      ...
      <nav>
        <ul>
          {breeds.map((breed, index) => (<button key={index} onClick={() => setSelectedBreed(breed)}>{breed}</button>))}
        </ul>
      </nav>
